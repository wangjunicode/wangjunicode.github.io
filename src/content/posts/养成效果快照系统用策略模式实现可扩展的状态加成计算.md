...
如果用一堆if-else堆砌各种特殊逻辑，代码会变成"效果地狱"。xgame项目用**效果快照（Effect Snapshot）模式**解决了这个问题。
...
xgame的`DataSource`设计支持分别累积`BasicValue`（倍率来源）和`Multiplier`（累积百分比），最终用公式统一计算，保证叠加顺序的数学正确性。
