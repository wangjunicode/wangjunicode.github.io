...
本文分析xgame项目的`BattleVoiceControlComponentSystem`和`BattleCommentaryVoiceComponentSystem`，深入了解战斗语音的完整机制。
...
解决方案：战斗结算时调用`xgameAudioManager.StopCategory(SoundType.Voice)`，停止所有语音类型音效。
