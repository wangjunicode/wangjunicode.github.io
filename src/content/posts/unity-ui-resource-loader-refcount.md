...
VGame项目的`VGameUILoader`用引用计数+并发合并机制优雅地解决了这些问题。本文深入剖析这套设计。

## 一、核心数据结构：VGameUILoadHandle

```csharp
public class VGameUILoadHandle : IRefPool
{
    ...
    private void Release()
    {
        ...
        VGameLoadHandleHelper.PutLoad(ResloadPath);
    }
...
`VGameUILoadHandle`是资源加载的句柄，与资源路径1对1绑定。它通过引用计数管理资源的生命周期：
...
```csharp
public static class VGameLoadHandleHelper
{
    // 路径 → Handle（用于查找"这个资源已经在加载/加载完了吗"）
    private static Dictionary<string, VGameUILoadHandle> m_AllLoad = new();
    
    // 对象 → Handle（用于通过资源对象反查Handle，进行引用计数管理）
    private static Dictionary<Object, VGameUILoadHandle> m_ObjLoadHandle = new();
}
```
...
{
    var load = VGameLoadHandleHelper.GetLoad(resLoadPath); // 获取或创建Handle
    ...
    // 注册对象→Handle的映射
    VGameLoadHandleHelper.AddLoadHandle(obj, load);
...
{
    var load = VGameLoadHandleHelper.GetLoad(resLoadPath);
    load.AddRefCount();
    var loadObj = load.Object;
    if (loadObj != null)
    {
