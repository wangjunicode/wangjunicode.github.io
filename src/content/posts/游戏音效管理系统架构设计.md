...
```
xgameAudioManager (门面层 / Facade)
    ↓  协调调度
xgameAudioManagerNew (战斗音频层)
    ↓  实例管理
xgameSndInstanceManager (实例池层)
    ↓
xgameSoundEngine (引擎对接层 - Wwise)
```

每一层的职责：
- **xgameAudioManager**：对外接口，处理 BGM、音效分组、音量设置
- **xgameAudioManagerNew**：战斗专用，处理音效优先级与分层调度
- **xgameSndInstanceManager**：音效实例的对象池，管理 `xgameSndInstance` 的借用与归还
- **xgameSoundEngine**：Wwise 引擎的封装，屏蔽底层 API 差异
...
`xgameAudioManagerNew` 引入了音频分级（`AudioLevel`）：
...
频繁的 `new` 和 `GC` 是手游的大敌，尤其是高频率触发的打击音效。`xgameSndInstanceManager` 实现了一个 `Queue` 式对象池：
...
{
    if (reserved.Add(id))
        xgameSoundEngine.RegisterGameObj(id);
}

public void RemoveReservedId(ulong id)
{
    if (reserved.Remove(id))
        xgameSoundEngine.UnregisterGameObj(id);
}
```
...
private xgameAudioManagerNew AudioNew;
```

系统通过 `isopenNewModel` 标志在"旧模式"和"新模式"之间切换。这是典型的**功能开关**（Feature Flag）设计：

- 新版本（`xgameAudioManagerNew`）正在灰度测试
- 旧版本作为回退方案
...
| 内存持续增长 | 音效实例未归还池 | 用 `xgameSndInstanceManager` 强制走对象池 |
